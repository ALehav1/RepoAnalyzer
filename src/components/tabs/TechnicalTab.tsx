...
export const TechnicalTab: React.FC<TechnicalTabProps> = ({ objectives, patterns }) => {
  return (
    <div className="space-y-6 p-6">
      <h2 className="text-xl font-bold mb-4">Technical Objectives</h2>
                  ...
                  {objective.relatedPatterns.map(patternId => {
                    ...
                    return (
                      <div key={patternId} className="bg-gray-50 p-2 rounded">
                        <div className="flex items-start justify-between">
                          ...
                          <div className="flex items-center space-x-2">
                            <span className={`text-xs px-2 py-1 rounded ${
                              pattern.confidence >= 0.9 ? 'bg-green-100 text-green-800' :
                              pattern.confidence >= 0.7 ? 'bg-yellow-100 text-yellow-800' :
                        ...
                        {pattern.implementation.modelConfig && (
                          <div className="mt-2 text-xs text-gray-600">
                            <span className="font-medium">Performance:</span> {pattern.implementation.modelConfig.performance.latency}ms latency,{' '}
                            {pattern.implementation.modelConfig.performance.throughput} req/s
                          </div>
                        )}
                      </div>
                    );
